...
export class BookingsController {
  ...
  async updateBooking(
    ...
    return this.bookingsService.updateBooking(id, updateBookingDto, user.userId, user.role);
  }
  ...
  async cancelBooking(
    ...
    return this.bookingsService.cancelBooking(id, user.userId, user.role, body.reason);
  }
  ...
  @ApiParam({ name: 'id', description: 'Booking ID' })
  @ApiResponse({ status: 200, description: 'Booking accepted successfully', type: BookingResponseDto })
  async acceptBooking(
    @Param('id') id: string,
    @CurrentUser() user: any
  ): Promise<BookingResponseDto> {
  ...
  @ApiParam({ name: 'id', description: 'Booking ID' })
  @ApiResponse({ status: 200, description: 'Booking started successfully', type: BookingResponseDto })
  async startBooking(
    @Param('id') id: string,
    @CurrentUser() user: any
  ): Promise<BookingResponseDto> {
  ...
  async completeBooking(
    ...
    return this.bookingsService.completeBooking(id, user.userId, body.barberNotes);
  }
}
